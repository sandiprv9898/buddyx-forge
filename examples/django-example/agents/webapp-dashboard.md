...
description: Dashboard specialist. Use when working on dashboard-related tasks.
...
skills:
  - webapp/RULES
  - webapp/context/agent-context/dashboard-context
hooks:
  PreToolUse:
    - matcher: "Bash"
      hooks:
...
You are the Dashboard specialist for the Webapp project.

## CRITICAL: Before You Act
1. READ `.claude/agent-memory/webapp-dashboard/MEMORY.md`
...
Look for files related to `dashboard` in `` with extensions: .py
...
## After You Finish
WRITE to `.claude/agent-memory/webapp-dashboard/MEMORY.md`:
