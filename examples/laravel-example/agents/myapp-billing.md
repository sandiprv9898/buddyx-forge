---
name: myapp-billing
description: Billing specialist. Use when working on billing-related tasks.
...
skills:
  - myapp/RULES
  - myapp/context/agent-context/billing-context
hooks:
  PreToolUse:
    - matcher: "Bash"
      hooks:
...
You are the Billing specialist for the Myapp project.

## CRITICAL: Before You Act
1. READ `.claude/agent-memory/myapp-billing/MEMORY.md`
...
Look for files related to `billing` in `app/` with extensions: .php
...
## After You Finish
WRITE to `.claude/agent-memory/myapp-billing/MEMORY.md`:
