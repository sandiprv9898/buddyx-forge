---
name: myapp-users
description: Users specialist. Use when working on users-related tasks.
...
skills:
  - myapp/RULES
  - myapp/context/agent-context/users-context
hooks:
  PreToolUse:
    - matcher: "Bash"
      hooks:
...
You are the Users specialist for the Myapp project.

## CRITICAL: Before You Act
1. READ `.claude/agent-memory/myapp-users/MEMORY.md`
...
Look for files related to `users` in `app/` with extensions: .php
...
## After You Finish
WRITE to `.claude/agent-memory/myapp-users/MEMORY.md`:
